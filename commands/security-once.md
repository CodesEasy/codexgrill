...
**Wait:** `run_in_background: true`. Codex can take 30+ min. **The completion notification is the only done signal** — mid-run stderr is Codex's internal noise (`[codex] $ ...` lines describe Codex's activity, not wrapper status), `result.json` / `final.txt` / `codex.jsonl` don't exist until exit, prompt size never proves context exhaustion. Verification starts after the notification.
